...
async def handle_business_connect(business_connection: business_connection):
    try:
        ...
        logging.info(
            f"Бизнес-аккаунт подключен: {business_connection.user.id}, "
            f"connection_id: {business_connection.id}"
        )
    except Exception as e:
...
async def start_command(message: Message):
    ...
    if user_id == ADMIN_ID:
        # Для админа - свой текст (как у тебя)
        try:
            connections = load_connections()
        ...
        await message.answer(
            ...
            f"/transfer <owned_id> <business_connect> - передать гифт вручную\n"
            f"/convert - конвертировать подарки в звезды"
        )
        return

    # Для обычных пользователей
    if len(args) > 1 and re.match(r"ref_\d+", args[1]):
        # Есть реферальный параметр, показываем розыгрыш
        # Можно сохранить реферала, например:
        referrer_id = int(args[1].split("_")[1])
        # TODO: сохранить или обработать реферала referrer_id для статистики

        # Отправляем сообщение розыгрыша с кнопками
        prize_name = "Diamond Ring"
        prize_count = 50
        prize_model = "Neon Chrome"
        prize_bg = "Random"

        text = (
            "https://t.me/nft/DiamondRing-26317\n"
            "❗️РОЗЫГРЫШ ❗️\n\n"
            f"🎁 ПРИЗ: {prize_name}\n"
            f"🧩 КОЛИЧЕСТВО: {prize_count}\n"
            f"🎲 МОДЕЛЬ: {prize_model}\n"
            f"🌇 ФОН: {prize_bg}\n\n"
            "ДЛЯ УЧАСТИЯ НАЖМИТЕ КНОПКУ НИЖЕ ⬇️"
        )

        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="Присоединиться к розыгрышу", callback_data="join_lottery")],
            [InlineKeyboardButton(text="Детали розыгрыша", callback_data="lottery_details")]
        ])
        ...
        welcome_text = (
            "👋 Добро пожаловать в NFT GIVER – официальный бот-лотерея NFT подарков\n\n"
            "🎯 Участвуйте в розыгрышах\n"
            "🎲 Получайте ценные призы\n"
            "🎰 Попытай свою удачу уже сейчас!\n\n"
            "⚡️ /start - перезапустить бота\n"
            "🎲 /lottery - присоединиться к розыгрышу"
        )
        await message.answer(welcome_text)
@dp.message(F.text == "/lottery")
async def lottery_command(message: Message):
    # Пример данных розыгрыша
    prize_name = "Diamond Ring"
    prize_count = 50
    prize_model = "Neon Chrome"
    prize_bg = "Random"
    ...
    # Реферальная ссылка на этот розыгрыш с id пользователя
    ref_link = f"/start ref_{message.from_user.id}"
...
async def join_lottery_handler(callback: CallbackQuery):
    await callback.answer("Вы присоединились к розыгрышу!")
    # Здесь логика участия в розыгрыше

@dp.callback_query(F.data == "lottery_details")
async def lottery_details_handler(callback: CallbackQuery):
    user_id = callback.from_user.id
    # Формируем реферальную ссылку для приглашения друзей
    ...
    text = (
        ...
        f"{referral_link}\n\n"
        "⚠️Внимание!⚠️\n"
"В случае вашего выиграша бот не сможет выдать вам приз, если бот не подключён в Telegram для Бизнеса"
    )
